...
public class CopyConstructorAndArgsLocator<T> extends ConstructorAndArgsLocator<T> {
    ...
    public CopyConstructorAndArgsLocator(final Class<T> elementClass,
        ...
        if (sourceOffsets.length != source.getDimensionCount()) {
    ...
    public ConstructorAndArgs<T> getForIndices(long[] indices) throws NoSuchMethodException {
        ...
        constructorAndArgs.getConstructorArgs()[0] = source.get(targetIndexes);
