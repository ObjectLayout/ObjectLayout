 ...
 */
public class UnifiedCopyConstructorAndArgsLocator<T> extends ConstructorAndArgsLocator<T> {

    final Constructor<T> copyConstructor;
    final UnifiedStructuredArray<T> source;
     ...
     */
    public UnifiedCopyConstructorAndArgsLocator(final Class<T> elementClass,
                                                final UnifiedStructuredArray<T> source) throws NoSuchMethodException {
    ...
    public ConstructorAndArgs<T> getForIndexes(long[] indexes) throws NoSuchMethodException {
        ...
        for (int i = 0; i < indexes.length; i++) {
            targetIndexes[i] = indexes[i] + sourceOffsets[i];
        }
    ...
    /**
     * Recycle an {@link ConstructorAndArgs} instance (place it back in the internal cache if
     * desired). This is [very] useful for avoiding a re-allocation of a new {@link ConstructorAndArgs}
     * and an associated args array for {@link #getForIndexes(long[])} invocation in cases such as this (where the
     * returned {@link ConstructorAndArgs} is not constant across indices).
