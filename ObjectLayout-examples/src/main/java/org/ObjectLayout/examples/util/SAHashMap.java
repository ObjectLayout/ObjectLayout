...
/* Ported from OpenJDK8's java.util.HashMap by Gil Tene.
 * Copyright (c) 2015, Gil Tene. All rights reserved.
 * Please note the LICENSE file in this directory, and the below
 * Oracle copyright, GPLv2 license notice, and Classpath EXception.
 */
/*
 ...
 * @since   1.2
 */
public class SAHashMap<K,V> extends SAAbstractMap<K,V>
    ...
    static class NodeTable<K,V> extends StructuredArray<Node<K,V>> {
        private static final MethodHandles.Lookup lookup = MethodHandles.lookup();

        static final Constructor<Node> nodeConstructor;
        static final CtorAndArgs<Node> nodeCtorAndArgs;
        static final Constructor<NodeTable> nodeTableConstructor;
        static final CtorAndArgs<NodeTable> nodeTableCtorAndArgs;
        static {
            try {
                nodeConstructor = Node.class.getDeclaredConstructor();
                nodeConstructor.setAccessible(true);
                nodeCtorAndArgs = new CtorAndArgs<Node>(nodeConstructor);
                nodeTableConstructor = NodeTable.class.getDeclaredConstructor();
                nodeTableConstructor.setAccessible(true);
                nodeTableCtorAndArgs = new CtorAndArgs<NodeTable>(nodeTableConstructor);
        ...
        public static <K,V> NodeTable<K,V> newInstance(final long length) {
            return StructuredArray.newInstance(nodeTableCtorAndArgs, nodeCtorAndArgs, length);
//            return StructuredArray.newInstance(lookup, NodeTable.class, Node.class, length);
        }
    }
...
//    transient Node<K,V>[] table;
    transient NodeTable<K,V> saTable;
     ...
     * @throws IllegalArgumentException if the initial capacity is negative.
     */
    public SAHashMap(int initialCapacity) {
     ...
     */
    public SAHashMap(Map<? extends K, ? extends V> m) {
...
//                    return ((TreeNode<K,V>)first).getTreeNode(hash, key);
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                   boolean evict) {
        NodeTable<K,V> tab; Node<K,V> p; int n, i;
        if ((tab = saTable) == null || (n = (int)tab.getLength()) == 0)
            n = (int)(tab = resize()).getLength();
        if ((p = tab.get(i = (n - 1) & hash)).isSentinel() == true)
//            tab[i] = newNode(hash, key, value, null);
            tab.get(i).setContents(hash, key, value, null);
        else {
            Node<K,V> e; K k;
            if (p.hash == hash &&
...
//                        ((TreeNode<K,V>)e).split(this, newTab, j, oldCap);
                        ...
                        if (loTail != null) {
                            loTail.next = null;
//                            newTab[j] = loHead;
                            newTab.get(j).setContents(loHead);
                        }
                        if (hiTail != null) {
                            hiTail.next = null;
...
//                tab[i] = null;
    ...
    final class KeySet extends AbstractSet<K> {
        ...
        public final void clear()               { SAHashMap.this.clear(); }
...
//        }
    ...
    @Override
    public Object clone() {
        SAHashMap<K,V> result;
        try {
            result = (SAHashMap<K,V>)super.clone();
...
//            Node<K,V>[] tab = (Node<K,V>[])new Node[cap];
    ...
    static class HashMapSpliterator<K,V> {
        ...
        HashMapSpliterator(SAHashMap<K,V> m, int origin,
                           int fence, int est,
        ...
        final int getFence() { // initialize fence and size on first use
            ...
            if ((hi = fence) < 0) {
                ...
                NodeTable<K,V> tab = m.saTable;
                hi = fence = (tab == null) ? 0 : (int)tab.getLength();
            }
            return hi;
        }
